...
Shader "Custom/PosEffectts" 
{
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		texUV += _Offset;
		texUV.x += (rand(floor(texUV.y * 5) + _Time.y) - 0.5) * _NoiseX;
		texUV = mod(texUV, 1);
		...
		// RGBノイズ
		/*if (rand((rand(floor(texUV.y * 50) + _Time.y) - 3.0) + _Time.y) < _RGBNoise)
		{
		...
		// スキャンラインを描画
		//float scanLineColor = sin(_Time.y * 10 + uv.y * 1000) / 2 + 0.5;
